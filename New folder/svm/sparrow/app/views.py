    ...
    if request.method=="POST":
        ...
        def p_dtls(request,pmasteradd):
            ...
            if request.method=="POST":
                ...
                total=request.POST['total']
                invoicedate=request.POST['invoicedate']
                p_dtls=p_temp(itamame=itemname,rate=rate,qty=qty,total=total,invoicedate=invoicedate)
                p_dtls.save()
                data=p_temp.objects.all()
